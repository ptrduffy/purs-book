...
showShape (Clipped pic c w h) =
  (foldl (<>) "" (map showShape pic)) <> " - clipped to rectangle" <> "[center: " <> showPoint c <> ", width: " <> show w <> ", height: " <> show h <> "]"
...
shapeBounds (Clipped pic c w h) =
  shapeBounds $ Rectangle { x: c.x, y: c.y } w h
